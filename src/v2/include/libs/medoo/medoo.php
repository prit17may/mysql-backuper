...
class medoo
{
            ...
            if (preg_match("/^(AND|OR)\s*#?/i", $key, $relation_match) && $type == 'array') {
                ...
                if (isset($match[3])) {
                    if ($match[3] == '') {
                        ...
                        break;
                        case 'string':$wheres[] = $column.' != '.$this->quote($value);
                        break;
                    }
                    } else {
                        if ($match[3] == '<>' || $match[3] == '><') {
                    ...
                    if (is_int($key)) {
                        ...
                        switch ($type) {case 'NULL':$wheres[] = $column.' IS NULL'; break; case 'array':$wheres[] = $column.' IN ('.$this->array_quote($value).')'; break; case 'integer':case 'double':$wheres[] = $column.' = '.$value; break; case 'boolean':$wheres[] = $column.' = '.($value ? '1' : '0'); break; case 'string':$wheres[] = $column.' = '.$this->quote($value); break; }
                    }
                }
            }
        }
    ...
    public function insert($table, $datas)
    {
        ...
        foreach ($datas as $data) {
            ...
            $index = 0;
            foreach ($data as $key=>$value) {
                switch (gettype($value)) {case 'NULL':$values[] = 'NULL'; break; case 'array':preg_match("/\(JSON\)\s*([\w]+)/i", $key, $column_match); if (isset($column_match[0])) {
                    $keys[$index] = $column_match[1];
                    $values[] = $this->quote(json_encode($value));
                } else {
                    $values[] = $this->quote(serialize($value));
                }break; case 'boolean':$values[] = ($value ? '1' : '0'); break; case 'integer':case 'double':case 'string':$values[] = $this->quote($value); break; }
                $index++;
            }
    ...
    public function update($table, $data, $where = null)
    {
        ...
        foreach ($data as $key=>$value) {
            ...
            if (isset($match[3])) {
                ...
                }break; case 'boolean':$fields[] = $column.' = '.($value ? '1' : '0'); break; case 'integer':case 'double':case 'string':$fields[] = $column.' = '.$this->quote($value); break; }
            }
        }
